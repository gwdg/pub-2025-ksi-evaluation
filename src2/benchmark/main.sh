...
if [ "$ONLY_PARSE" == "false" ]; then
  ...
  for i in {01..10}; do ../ksi/run-workload-original.sh ../ksi-evaluation/src2/benchmark/iperf3-bandwidth.sh 2>&1 > logs2/iperf3-bandwidth/podman-pasta/$i.txt; done
  for i in {01..10}; do ../ksi/run-workload-no-liqo.sh ../ksi-evaluation/src2/benchmark/iperf3-bandwidth.sh 2>&1 > logs2/iperf3-bandwidth/iperf3/nerdctl-bypass4netns/$i.txt; done
  for i in {01..10}; do ../ksi/run-workload-no-liqo.sh ../ksi-evaluation/src2/benchmark/iperf3-bandwidth.sh 2>&1 > logs2/iperf3-bandwidth/iperf3/nerdctl-bypass4netns/$i.txt; done
